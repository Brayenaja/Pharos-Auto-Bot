...
const checkBalanceAndApproval = async (wallet, tokenAddress, amount, decimals, spender) => {
  try {
    ...
    if (allowance < required) {
      ...
      const txOptions = await buildTxOptions(wallet, gasLimit);
...
const transferPHRS = async (wallet, provider, index, jwt, proxy) => {
  try {
    ...
    const feeData = await provider.getFeeData();
    const gasPrice = feeData.gasPrice || ethers.parseUnits('1', 'gwei');
    const txOptions = await buildTxOptions(wallet, 21000);
    const tx = await wallet.sendTransaction({ to: toAddress, value: required, ...txOptions });
...
const buildTxOptions = async (wallet, gasLimit) => {
  const feeData = await wallet.provider.getFeeData();
